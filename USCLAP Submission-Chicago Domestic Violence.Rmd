...
ggplot(data = domvio_perday, mapping = aes(x = Var1, y = DOMVIOCASES, colour = period_indicator, group=1)) + geom_line()+ geom_point() +
  labs(title = "# Daily domestic violence cases
  remained relatively constant",
       x = "MM/YYYY",
       y = "daily cases") + expand_limits(x = 0, y = 0) + scale_x_discrete(breaks = c(20190901, 20191201,20200301, 20200601), label = c("09/2019", "12/2019", "03/2020", "06/2020")) + theme(plot.margin = unit(c(4,0,0,9),"cm"), legend.position = "none")

ggplot(data = domvio_perday, mapping = aes(x = Var1, y = TOTCASES, colour = period_indicator, group=1)) + geom_line() + geom_point() + geom_label(x=200, y=1700, label="May 31st BLM protest", show.legend = F, color = "black") +
  labs(title = "Total # daily crime cases dipped
       at the onset of lockdown",
       x = "MM/YYYY"
       ) + expand_limits(x = 0, y = 0) + scale_x_discrete(breaks = c(20190901, 20191201,20200301, 20200601), label = c("09/2019", "12/2019", "03/2020", "06/2020")) + theme(plot.margin = unit(c(4,0,0,5),"cm"), axis.title.y=element_blank())
```
...
Though our analyses show increases in the proportion and likelihood of reported domestic violence cases, we cannot conclude that this is a direct result of an actual rise in cases of domestic violence. Most likely, the decrease in total crime cases during the lockdown (as reported by some sources [9]) was more significant than the decrease in domestic violence cases, resulting in an increase in the proportion of domestic violence crimes. it might be worthwhile to conduct an ANOVA study in order to determine if the mean number of cases of domestic violence reported per day actually changed throughout the phases of the pandemic.
Were we to repeat our analyses, we would adjust for more confounders to improve the prediction accuracy of our model, expand the population of interest, control for the abnormal spikes and dips in our data resulting from protests, and conduct research on whether police report data is actually reflective of the true number of domestic violence cases by examining other methods of reporting such as domestic violence hotlines. Finally, it would be interesting to follow rates of domestic violence for an extended time period after the immediate threat of the virus has passed, and we are left to deal with its long-lasting consequences.
\newpage
...
BEFORE-LOCKDOWN (January 24, 2020 - The first confirmed case in Illinois (a Chicago resident) is reported [4]).
LOCKDOWN (March 22, 2020 - First full day of strict stay at home order for all individuals except those considered essential workers [5]).
2. PHASE 2 (May 1, 2020 - Chicago enters phase 2 of its gradual reopening plan, under a modified stay at home order that allows some non-essential businesses to reopen for curb-side pickup and delivery, in addition to allowing residents to resume some outdoor activities while practicing social distancing [6]).
3. PHASE 3 (June 3, 2020 - Chicago enters phase 3 of its gradual reopening plan, which allows for gatherings of up to 10 and reopened offices, retail stores, bars, barbershops, and churches, all with safety precautions [5]).
4. PHASE 4 (June 26, 2020 - Chicago enters phase 4 of its gradual reopening plan, which reopens schools, restaurant outdoor seating, and fitness clubs with safety precautions in place [7]).
...
## APPENDIX C: Logistic Regression Output and Predictor Odd Ratio Confidence Intervals
